...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Breadium app in browser mode", w.failure
	}
	if w.offline() {
		return "Breadium app offline", w.failure
	}
	return fmt.Sprintf("Breadium app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}
