...
import (
	...
	"github.com/blessingreacoin/go-breadiuma/eth/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Breadium peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Breadium protocol")
	...
	log.Info("Breadium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Breadium peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Breadium handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Breadium peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Breadium message handling failed", "err", err)
			return err
		}
	}
}
