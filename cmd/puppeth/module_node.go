...
{{end}}
RUN \
  echo 'gbrea --cache 512 init /genesis.json' > gbrea.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.breadium/keystore/ && cp /signer.json /root/.breadium/keystore/' >> gbrea.sh && \{{end}}
	echo $'exec gbrea --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gbrea.sh

ENTRYPOINT ["/bin/sh", "gbrea.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.breadium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.breahash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.breadium"],
		ethashdir:  infos.volumes["/root/.breahash"],
		port:       port,
